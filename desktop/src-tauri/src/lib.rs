...
fn pty_start(
    ...
    // Per resolve_sidecar_paths' invariant, both are already stripped of the
    // `\\?\` verbatim prefix on Windows — no per-consumer normalization needed.
    ...
    // Both relative and absolute paths to openclaw.mjs trigger `EISDIR lstat
    // 'C:'` from Node's resolveMainPath inside ConPTY on some Windows installs
    // (reproducible when the perMachine install puts openclaw.mjs under
    // `C:\Program Files\OpenClaw\...`). Workaround: write a tiny launcher to
    // %TEMP% (which has no spaces) and have it `import(file://...)` the real
    // openclaw.mjs — Node only needs to resolve the launcher path as the main
    // script, and the file:// URL import bypasses Module._findPath entirely.
    let script_path = openclaw_dir.join("openclaw.mjs");
    let main_script = match write_pty_launcher(&script_path) {
        Ok(path) => {
            eprintln!("[pty] launcher: {} -> {}", path.display(), script_path.display());
            path
        }
        Err(err) => {
            eprintln!("[pty] launcher write failed ({err}) — falling back to direct script");
            script_path.clone()
        }
    };
...
// triggers `EISDIR lstat 'C:'` under ConPTY. The launcher rewrites
...
fn write_pty_launcher(real_script: &Path) -> std::io::Result<PathBuf> {
    // Caller invariant: real_script is already prefix-free (resolve_sidecar_paths
    // strips `\\?\` at the producer). The launcher itself is written under
    // %TEMP% (typically `C:\Users\<user>\AppData\Local\Temp\…` — no spaces and
    // no verbatim prefix), so Node's resolveMainPath of THIS file resolves
    // cleanly; the real openclaw.mjs is then loaded via file:// URL which uses
    // WHATWG URL semantics instead of Module._findPath / realpath.
    ...
    // try/catch around the dynamic import: without it a thrown error inside
    // openclaw's top-level await chain surfaces as a generic
    // UnhandledPromiseRejection that swallows the actual cause. The explicit
    // catch + console.error gives us (and the user) a meaningful first line.
    let content = format!(
        "// Auto-generated by OpenClaw desktop shell. Spoofs argv so the real\n\
         // openclaw.mjs sees argv[1] == its own absolute path, then imports it\n\
         // via file:// URL (no realpath of the install path → no EISDIR).\n\
         process.argv = [process.argv[0], {argv1_lit}, ...process.argv.slice(2)];\n\
         try {{ await import({url_lit}); }}\n\
         catch (e) {{ console.error('[openclaw-launcher]', (e && e.stack) || e); process.exit(1); }}\n\
        ",
    );
...
// Drive-rooted path:  `C:\Program Files\…\openclaw.mjs`     → `file:///C:/Program%20Files/…/openclaw.mjs`
// UNC-rooted path:    `\\server\share\openclaw\openclaw.mjs` → `file://server/share/openclaw/openclaw.mjs`
//
// Only encodes characters that are unsafe in a URL path component. Drive letter
// + colon stays unescaped (legal per WHATWG URL spec for host-less file URLs).
// Caller MUST pass a verbatim-prefix-free path; resolve_sidecar_paths' invariant
// guarantees this and the debug_assert below catches regressions.
fn path_to_file_url(p: &Path) -> String {
    let raw = p.to_string_lossy();
    debug_assert!(
        !raw.starts_with(r"\\?\") && !raw.starts_with(r"\??\"),
        "path_to_file_url received a verbatim-prefixed path: {raw:?}",
    );
    let s = raw.replace('\\', "/");
    // UNC `\\server\share\...` becomes `//server/share/...` after the replace.
    // file:// URL spec wants `file://server/share/...` (two slashes, NOT three),
    // so we emit "file:" without trailing slashes when the path is UNC.
    // For everything else (drive-rooted), use "file:///" (three slashes).
    ...
    for c in s.chars() {
        if c.is_ascii_alphanumeric() || matches!(c, '/' | ':' | '-' | '_' | '.' | '~') {
            ...
            // Non-ASCII (rare in install paths). UTF-8 encode each byte.
            let mut buf = [0u8; 4];
...
mod path_tests {
    use super::*;
    #[test]
    fn strips_known_verbatim_prefixes() {
        assert_eq!(strip_verbatim_prefix(Path::new(r"\\?\C:\Program Files\OpenClaw")), PathBuf::from(r"C:\Program Files\OpenClaw"));
        assert_eq!(strip_verbatim_prefix(Path::new(r"\??\C:\foo")), PathBuf::from(r"C:\foo"));
        assert_eq!(strip_verbatim_prefix(Path::new(r"\\?\UNC\server\share\openclaw")), PathBuf::from(r"\\server\share\openclaw"));
        assert_eq!(strip_verbatim_prefix(Path::new(r"C:\already\clean")), PathBuf::from(r"C:\already\clean"));
    }
    #[test]
    fn file_url_drive_rooted_with_spaces() {
        assert_eq!(
            path_to_file_url(Path::new(r"C:\Program Files\OpenClaw\openclaw.mjs")),
            "file:///C:/Program%20Files/OpenClaw/openclaw.mjs"
        );
    }
    #[test]
    fn file_url_unc_does_not_get_extra_slash() {
        let out = path_to_file_url(Path::new(r"\\server\share\openclaw.mjs"));
        assert_eq!(out, "file://server/share/openclaw.mjs");
...
// AppState invariant: every PathBuf this function emits is in plain Win32 form
// (no `\\?\…` verbatim prefix). Tauri 2's app.path().resource_dir() AND
// std::env::current_exe() can both return verbatim-prefixed paths on Windows
// (perMachine NSIS install under `C:\Program Files\…`). Letting that prefix
// leak downstream broke (a) Node's resolveMainPath / realpath with `EISDIR
// lstat 'C:'`, and (b) our path_to_file_url with malformed
// `file://///%3F/C:/…`. Strip at this single producer so every consumer —
// spawn_gateway, ensure_openclaw_config, run_openclaw_cli, write_pty_launcher,
// read_openclaw_version — is automatically inoculated.
fn resolve_sidecar_paths(app: &tauri::AppHandle) -> tauri::Result<(PathBuf, PathBuf)> {
    ...
    if let Ok(rd) = app.path().resource_dir() {
        let rd = strip_verbatim_prefix(&rd);
    ...
    for base in &candidates {
        ...
        if n.exists() {
            // Defense in depth — base is already stripped, but `.join` can't
            // re-introduce a prefix so this is cheap.
            return Ok((strip_verbatim_prefix(&n), strip_verbatim_prefix(&base.join("openclaw"))));
        }
    }
    ...
    Ok((
        strip_verbatim_prefix(&fallback.join("node").join(node_exe)),
        strip_verbatim_prefix(&fallback.join("openclaw")),
    ))
}
